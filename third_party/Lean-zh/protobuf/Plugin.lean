...
def generate_code (request : CodeGeneratorRequest) : ExceptT String IO CodeGeneratorResponse := do
  ...
  let renderFile (imports : Array String) (cmds : Array Command) (descriptors : String)
    ...
    -- Embedded reflection descriptors reference Grpc.Services.Reflection, so
    -- every generated module imports Grpc (lean_proto_library always depends
    -- on the gRPC runtime library).
    let grpcImport := "public import Grpc"
  ...
  for name in filesToGenerate do
    ...
    let services ← liftM (m := Except String) (n := ExceptT String IO) <|
