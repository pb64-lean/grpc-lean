...
private def field_default_value (t : FieldDescriptorProto.Type) (raw_value : String) :
    M (TSyntax `options_value × Bool) := do
  match t with
  | .TYPE_STRING =>
      let b64 := Protobuf.Base64.encode raw_value.toUTF8
      let lit : TSyntax `str := ⟨Lean.Syntax.mkStrLit b64⟩
      return (← `(options_value| $lit:str), true)
  | .TYPE_BYTES =>
      let b64 := Protobuf.Base64.encode raw_value.toUTF8
      let lit : TSyntax `str := ⟨Lean.Syntax.mkStrLit b64⟩
      return (← `(options_value| $lit:str), true)
  | .TYPE_BOOL =>
      match raw_value.trimAscii.toString with
      | "true" => return (← `(options_value| true), false)
      | "false" => return (← `(options_value| false), false)
      | _ => throw s!"{decl_name%}: invalid boolean default value '{raw_value}'"
  | .TYPE_ENUM =>
      let id := Lean.mkIdent (Name.mkStr1 raw_value.trimAscii.toString)
      return (← `(options_value| $id:ident), false)
  | .TYPE_DOUBLE
  | .TYPE_FLOAT
  | .TYPE_INT64
  | .TYPE_UINT64
  ...
  | .TYPE_SINT32
  | .TYPE_SINT64 =>
      return (← options_value_of_number raw_value, false)
  | .TYPE_MESSAGE =>
...
private def field_default_option? (field : FieldDescriptorProto) : M (Option (TSyntax ``options_entry)) := do
  ...
  let t ← get!! field.type
  let value ← field_default_value t raw_value
  let (value, isBase64) := value
  if isBase64 then
    some <$> `(options_entry| default_base64 = $value)
  else
    some <$> `(options_entry| default = $value)

private def field_options? (field : FieldDescriptorProto) : M (Option (TSyntax ``options)) := do
  let mut entries := #[]
  if field.type matches some .TYPE_GROUP then
    entries := entries.push (← `(options_entry| wired_as_group = true))
  if field.type matches some .TYPE_ENUM then
    entries := entries.push (← `(options_entry| closed_enum = true))
...
private def add_field_option (opts? : Option (TSyntax ``options)) (entry : TSyntax ``options_entry) :
    M (Option (TSyntax ``options)) := do
  match opts? with
  | none => some <$> `(options| [$entry])
  | some opts =>
      let `(options| [$entries,*]) := opts
        | throw s!"{decl_name%}: internal error: unexpected options syntax"
      some <$> `(options| [$entries,*, $entry])
...
partial def compile_message (item : MsgItem) : M DeclOutput := do
  ...
  for field in item.normalFields do
    ...
    if let some map_type ← map_field_type? item map_entries field then
      let mut fieldOpts ← field_options? field
      let some entry ← map_entry_desc? map_entries field
        | throw s!"{decl_name%}: internal error: map field is missing map entry descriptor"
      let (_, value_field) ← map_entry_fields entry
      if value_field.type matches some .TYPE_ENUM then
        fieldOpts ← add_field_option fieldOpts (← `(options_entry| closed_enum = true))
      opts := opts.push fieldOpts
