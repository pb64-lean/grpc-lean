...
metadata errors continue to outrank HTTP 415 and gRPC semantic rejections.
An opaque selector chooses one noinline wrapper before warmup or measurement,
and the common recurrence contains only the indirect classifier call plus a
complete result digest.

Every directed fixture is checked against an independent expected result
before selection.  The fixture set covers the exact
...
HTTP-415 precedence, duplicate singleton fields, ordered semantic errors,
pseudo-header layout errors, and ordinary metadata failures before HTTP 415.
...
namespace Grpc.RequestValidationFusionBenchmarkHarness
...
@[noinline] private opaque selectClassifierBox (mode : Mode) : ClassifierBox :=
  match mode with
  | .separate => {
      classify := classifySeparate
      fusedSelected := false
    }
  | .fused => {
      classify := classifyFused
      fusedSelected := true
    }
...
private def makeFixtures : Array Fixture :=
  ...
  let wideUnknown64 :=
    ((requestPrefix "/benchmark.Service/WideUnknown").append #[
      header "content-type" "application/grpc",
      header "te" "trailers"
    ]).append (fillerHeaders 58)
  let validBinary := (requestPrefix "/benchmark.Service/ValidBinary").append #[
    header "content-type" "application/grpc",
    header "te" "trailers",
  ...
  let unsupportedThenInvalidAscii :=
    ((requestPrefix "/benchmark.Service/InvalidAscii").append #[
      header "content-type" "application/json",
      header "te" "trailers"
    ]).push (header "x-control" "\u0000")
  let unsupportedThenForbidden :=
    ((requestPrefix "/benchmark.Service/Forbidden").append #[
      header "content-type" "application/json",
      header "te" "trailers"
    ]).push (header "connection" "keep-alive")
  let unsupportedThenInvalidBinary :=
    ((requestPrefix "/benchmark.Service/InvalidBinary").append #[
      header "content-type" "application/json",
      header "te" "trailers"
    ]).push (header "x-bin" "%%%")
  let invalidMethodThenInvalidAscii :=
    ((requestPrefix "/benchmark.Service/MethodThenMetadata" "GET").append #[
      header "content-type" "application/grpc",
      header "te" "trailers"
    ]).push (header "x-control" "\u0000")
  #[
    ...
    {
      label := "wide_unknown_64"
      metadata := wideUnknown64
      expected := accepted "benchmark.Service" "WideUnknown" none none false false
      expectedHeaders := 64
    },
    {
      label := "valid_binary_8"
      metadata := validBinary
      expected := accepted "benchmark.Service" "ValidBinary" none none false false
      expectedHeaders := 8
    },
    {
      label := "late_pseudo_after_regular_7"
      metadata := latePseudoAfterRegular
      expected := rejectedInvalid
        "HTTP/2 pseudo-header :authority appeared after regular metadata"
      expectedHeaders := 7
    },
    {
      label := "late_duplicate_pseudo_7"
      metadata := lateDuplicatePseudo
      expected := rejectedInvalid "duplicate HTTP/2 pseudo-header :scheme"
      expectedHeaders := 7
    },
    {
      label := "unsupported_then_invalid_ascii_7"
      metadata := unsupportedThenInvalidAscii
      expected := rejectedInvalid "invalid ASCII gRPC metadata value for x-control"
      expectedHeaders := 7
    },
    {
      label := "unsupported_then_forbidden_7"
      metadata := unsupportedThenForbidden
      expected := rejectedInvalid
        "HTTP/2 connection-specific metadata is forbidden: connection"
      expectedHeaders := 7
    },
    {
      label := "unsupported_then_invalid_binary_7"
      metadata := unsupportedThenInvalidBinary
      expected := rejectedInvalid
        "invalid binary gRPC metadata x-bin: invalid base64 character"
      expectedHeaders := 7
    },
    {
      label := "invalid_method_then_invalid_ascii_7"
      metadata := invalidMethodThenInvalidAscii
      expected := rejectedInvalid "invalid ASCII gRPC metadata value for x-control"
      expectedHeaders := 7
    }
  ]
...
private def validateFixtures (fixtures : Array Fixture) : IO Nat := do
  for fixture in fixtures do
    ...
    let reference := classifySeparate fixture.metadata
    let candidate := classifyFused fixture.metadata
...
private def modeIsFused : Mode → Bool
  | .separate => false
  | .fused => true
...
private def runFixture (classify : @& Classifier) (mode : Mode)
  ...
  IO.println <|
    s!"benchmark=grpc_request_validation_fusion_v1 mode={modeName mode} " ++
      s!"fixture={fixture.label} metadata_headers={fixture.metadata.size} " ++
      s!"iterations={iterations} warmup={warmup} digest={digest}"
  IO.println <|
    s!"request_validation_fusion_validation=pass cases={semanticCases} " ++
      "result=exact_separate_fused_expected digest=validation_and_all_outcome_fields"
  IO.println <|
    "counter_scope=whole_process fixed=fixture_construction+directed_semantic_validation+" ++
      ...
      "scaling=selected_validation_and_request_preflight+full_result_digest " ++
...
def runMain (args : List String) : IO Unit := do
  let (modeText, selection, iterations, warmup) ← match args with
    ...
    | _ => throw (IO.userError <|
        "usage: request_validation_fusion_benchmark (separate|fused) " ++
          "<fixture|all> iterations [warmup]")
  let some mode := parseMode modeText
    | throw (IO.userError "mode must be separate or fused")
  ...
  unless classifierBox.fusedSelected == modeIsFused mode do
...
end Grpc.RequestValidationFusionBenchmarkHarness

def main (args : List String) : IO Unit :=
  Grpc.RequestValidationFusionBenchmarkHarness.runMain args
